...
bool Area::AddaReaction(int i ,Post p)
{
    if (i < this->Posts.size()) {
        this->Posts[i].setLikes(p.getLikes());
        this->Posts[i].setHates(p.getHates());
        this->Posts[i].setDislikes(p.getDislikes());
        this->Posts[i].setLoves(p.getLoves());
        return true;
    } else { return false; }
}
